...
data XaToken = token(str chars) | space(str chars) | comment(str chars) | child(int index) | sep(XaToken tok, list[XaToken] seperator);
...
public rel[str, int, list[XaToken], Production] grammar2info(set[Production] prods) {
	rel[str, int, list[XaToken], Production] tbl = {};
	for (p <- prods) {
		s = sortName(p);
		if (hasCons(p)) {
			<toks, i> = concrete(p);
			tbl += {<consName(p), i, toks, p>};
		} 
    }
  return tbl;
}
