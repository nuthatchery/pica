...
public class ResourceManager implements IResourceChangeListener,
	...
	private void openProject(IProject project) {
		...
		projectContents.put(project, paths);
		projects.put(project.getName(),
				new ProjectManager(this, project, paths));
	...
	private void initializeTransaction() {
		...
		tr = new Transaction(new INameFormatter() {
			...
			public String format(IValue name) {
				if(TermAdapter.isCons(name, "QName")
						|| TermAdapter.isCons(name, "Name"))
			...
			for(IPath p : entry.getValue()) {
				if(resources.get(p) == null)
					throw new ImplementationError(
							"Data invariant check failed: " + p
									+ " is member of project " + entry.getKey()
	...
	public IPath getPath(URI uri) {
		if(uri.getScheme().equals("project")) {
			...
			throw new IllegalArgumentException("Only project URIs are handled!");
		}
	}
